# =============================================================================
# --- ARQUIVO: ui/dialogs_tools.py (REMOVIDA JANELA DE CONFIG. DE E-MAIL) ---
# =============================================================================
...
class SettingsWindow(ttk.Toplevel):
    ...
    def select_xml_path(self):
        ...
        if folder_path: self.xml_path_var.set(folder_path)
...
class ValidatorWindow(ttk.Toplevel):
    ...
    def validate(self):
        ...
        if len(doc) == 11 and self._validate_cpf(doc):
            self.result_var.set("CPF Válido!"); self.result_label.config(bootstyle="success")
        elif len(doc) == 14 and self._validate_cnpj(doc):
            self.result_var.set("CNPJ Válido!"); self.result_label.config(bootstyle="success")
        else:
            self.result_var.set("Número Inválido!"); self.result_label.config(bootstyle="danger")
...
class KeyParserWindow(ttk.Toplevel):
    def __init__(self, controller):
        ...
        self.copy_button = ttk.Button(main_frame, text="Copiar Resultado", command=self.copy_to_clipboard, state="disabled")
        self.copy_button.grid(row=3, column=0, pady=(5,0), sticky='e')
    ...
    def parse_key(self):
        ...
        if len(key) != 44:
            ...
            self.text_area.config(state='disabled'); self.copy_button.config(state="disabled")
            return
        parts = { "UF do Emitente (cUF)": key[0:2], "Ano e Mês de Emissão (AAMM)": key[2:6], "CNPJ do Emitente": key[6:20], "Modelo (mod)": key[20:22], "Série (serie)": key[22:25], "Número da NF (nNF)": key[25:34], "Forma de Emissão (tpEmis)": key[34:35], "Código Numérico (cNF)": key[35:43], "Dígito Verificador (cDV)": key[43:44] }
        result_text = "--- Análise da Chave de Acesso ---\n\n"
        for label, value in parts.items(): result_text += f"{label:<30}: {value}\n"
        self.text_area.insert(tk.END, result_text)
        self.text_area.config(state='disabled'); self.copy_button.config(state="normal")

    def copy_to_clipboard(self):
        text_to_copy = self.text_area.get("1.0", "end-1c")
        if text_to_copy and "Aguardando chave" not in text_to_copy:
            self.clipboard_clear(); self.clipboard_append(text_to_copy)
            original_text = self.copy_button.cget("text")
            self.copy_button.config(text="Copiado!", bootstyle="success")
            self.after(2000, lambda: self.copy_button.config(text=original_text, bootstyle="normal"))